...
export default function TransactionItem({ item, type, onEdit, onDelete }: Props) {
  ...
  const valColor = isExtra ? 'var(--accent-green)' : 'var(--text-primary)'
  const prefix   = isExtra ? '+' : '-'

  const displayDate = item.transaction_date ? isoToBR(item.transaction_date) : '-'
  const initial = (item.description || '?').trim().charAt(0).toUpperCase()
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', gap: 6, alignItems: 'center' }}>
            <span>{item.bank || '—'}</span>
            {isFixo    && <><span className="dot" />FIXO</>}
            {isParcela && <><span className="dot" />{item.installment}</>}
          </div>
        </div>

        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: valColor }}>
